...
class ShopwareProductProduct(models.Model):
    ...
    _description = 'Shopware Product Product'
    _rec_name = 'name'
...
class ProductProductAdapter(GenericAdapter):
    ...
    def _call(self, method, arguments):
        try:
            ...
            # this is the error in the shopwareCommerce API
    ...
    def search(self, filters=None, from_date=None, to_date=None):
        ...
        return super(ProductProductAdapter, self).search(filters)

    def get_images(self, media_id):
        """
        This method is to ftech the product images from Shopware
        """
        arguments = {}
        return self._call('media/' + str(media_id), arguments)


@shopware
class ProductBatchImporter(DelayedBatchImporter):

    """ Import the shopwareCommerce Partners.
...
@shopware
class ProductProductImporter(ShopwareImporter):
    _model_name = ['shopware.product.product']
    ...
    def _create(self, data):
        """ Hook called at the creation of Openerp record """
        openerp_binding = super(ProductProductImporter, self)._create(data)
        # Adding Checkpoints for product creation
    ...
    def _after_import(self, binding):
        ...
        image_importer.run(self.shopware_id, binding, self.shopware_record)
        return
...
class ProductImageImporter(Importer):
    ...
    def _get_images(self, media_id):
        return self.backend_adapter.get_images(media_id)

    def _get_binary_image(self, image_data):
        url = image_data['path'].encode('utf8')
        url = str(url).replace("\\", '')
        try:
    ...
    def run(self, shopware_id, binding_id, record):
        self.shopware_id = shopware_id
        if record['images']:
            # Choosing first image, When there are multiple images.
            image = record['images'][0]['mediaId']
            # Fetching the image data
            image_data = self._get_images(image)
            # Converting as binary
            binary = None
            ...
            binding = model.browse(binding_id.id)
...
class ProductImportMapper(ImportMapper):
    ...
    # To be verify the exact fields with Shopware data.
    ...
    def price(self, record):
        ...
        prices = record['mainDetail']['prices']

        for price in prices:
            if price['from'] == 1 and price['customerGroup']['key'] == 'EK':
                return {'list_price': price['price']}

        raise MappingError("Could not store the price for the article detail\
         with shopware id %s" % record['id'])

    @mapping
    def standard_price(self, record):
        return {'standard_price': record['mainDetail']['purchasePrice']}
...
# Will remove the below source later.
